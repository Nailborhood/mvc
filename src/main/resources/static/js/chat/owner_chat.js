document.addEventListener('DOMContentLoaded', function () {
    ...
    function connect() {
        var socket = new SockJS('/stomp/chat'); // 서버의 설정과 일치해야 합니다.
        stompClient = Stomp.over(socket);
        stompClient.connect({}, function (frame) {
            console.log('Connected: ' + frame);
            stompClient.subscribe('/sub/chatroom/' + roomId, function (messageOutput) { // roomId는 현재 채팅방 ID입니다.
    ...
    function sendMessage() {
        ...
        if (messageContent && stompClient) {
            var chatMessage = {
                roomId: roomId, // 현재 채팅방 ID를 지정합니다.
                contents: messageContent,
                // TODO: writer는 서버 측에서 세션 정보를 통해 가져오게 바꾸기
                writer: writer,
            };
            stompClient.send("/pub/chatroom/" + roomId, {}, JSON.stringify(chatMessage)); // 서버의 설정과 일치해야 합니다.
...
// ownwer -mychat
    function showMessageOutput(messageOutput) {
        ...
        chatWrapper.classList.add(messageOutput.writer === 'ADMIN' ? 'userChat' : 'myChat');
